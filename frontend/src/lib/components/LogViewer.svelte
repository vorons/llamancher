<script lang="ts">
  import { Terminal, RefreshCw, Copy, X } from '@lucide/svelte';
...
{#if open}
  ...
  <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
  <div
    class="fixed bottom-0 left-0 right-0 z-50 h-64 md:h-80 border-t border-border bg-popover text-foreground font-mono text-xs flex flex-col"
  ...
  >
    ...
    <div class="flex items-center justify-between px-3 py-1.5 border-b border-border bg-card shrink-0">
      <div class="flex items-center gap-2">
        ...
        <span class="text-[11px] text-foreground font-medium uppercase tracking-wider">{$t('log.title')}</span>
        {#if lines.length > 0}
      ...
      <div class="flex items-center gap-1">
        <button
          class="flex items-center justify-center h-6 w-6 rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
          onclick={copyAll}
          title={$t('log.copyAll')}
        >
          <Copy size={12} />
    ...
    <!-- Status bar -->
    <div class="flex items-center gap-3 px-3 py-1 border-t border-border bg-card text-[10px] text-muted-foreground shrink-0">
